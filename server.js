...
if (process.env.MONGODB_URI) {
  // Render 這類雲端環境偶爾會跟 MongoDB Atlas 在 TLS 交握時出現
  // 「SSL alert number 80」這個錯誤，常見原因是 Node.js 新版 OpenSSL 3.x
  // 走 IPv6 出站連線時跟 Atlas 的相容性問題。這裡明確指定用 IPv4(family:4)
  // 連線、拉長逾時時間，是目前社群回報最有效的解法。
...
const LOADED_SAVE_GRACE_MS = 60 * 60 * 1000; // 從存檔重開的房間，座位保留久一點(1小時)，因為玩家們可能得先互相協調才會陸續回來
...
function getPublicRoomState(room) {
  return {
    players: Array.from(room.players.values()).map(p => ({ id:p.id, name:p.name, isHost:p.isHost, ready:p.ready, characterId:p.characterId, disconnected:!!p.disconnected, isAI:!!p.isAI })),
    hostId: room.hostId,
...
io.on('connection', (socket) => {
  ...
  socket.on('join_room', ({ roomCode, playerName, playerToken }, callback) => {
    ...
    // 保險機制：如果帶進來的playerToken剛好跟房間裡「已經存在」的某個玩家
    // token一樣(不管那個玩家目前是連著的還是還在斷線保留期)，代表這其實
    // 是同一個人想重新連回來，不是真的要當一個全新的人加入。直接導去跟
        // rejoin_room 一樣的「認回原本座位」邏輯，換上新的socket id繼續用同一筆
    // 玩家資料——不這樣做的話，遇到玩家的本地session被清掉(例如按了棋盤
    // 上的「退出遊戲」)又用房間代碼手動重新加入這種情況，會在room.started
    // 還沒擋下來之前，兜出「同一個人有兩筆资料」的重複角色。
    if (playerToken) {
      for (const [sid, p] of room.players.entries()) {
        if (p.token === playerToken) {
          clearTimeout(p.graceTimer);
          if (sid !== socket.id) room.players.delete(sid);
          p.id = socket.id;
          p.disconnected = false;
          p.disconnectedAt = null;
          p.graceTimer = null;
          room.players.set(socket.id, p);
          if (room.hostId === sid) room.hostId = socket.id;
          socket.join(roomCode);
          socket.data.roomCode = roomCode;
          socket.data.playerToken = playerToken;
          console.log(`[加入時偵測到既有座位，改走認回] ${roomCode} ${p.name}`);
          broadcastRoomState(roomCode);
          mpCheckAllSeatsClaimedFromSave(roomCode);
          return callback({
            ok: true, roomCode, playerToken, state: getPublicRoomState(room),
            mpStarted: !!room.mp,
    ...
    // 房間如果已經開局(不管是讀檔開的房、還是一般正常開局的房)，原本的其他
    // 玩家用一般加入房間的方式進來、但身份代碼對不上(例如清過本機資料、換了
    // 裝置、或從開場畫面重新讀檔進來)——之前這裡只有「讀檔房」才會被導去正確
    // 的認領座位流程，一般正常開局的房間會直接被下面「room.started」那條擋下
    // 來，誤判成「遊戲已經開始了，無法加入」，導致玩家真的沒辦法用這個方式
    // 認回自己的座位。這裡改成只要這個房間有正在進行的棋盤遊戲(room.mp存在)
    // 就一律導去認領座位流程，不限定一定要是讀檔房。
    if (room.mp) {
      const seatOrder = room.mp.seatOrder;
      // 座位一開始就帶著存檔當時的原始token(不是空的)，「還可以被認領」要用
      // 連線狀態動態判斷(沒有連線、或連線正在斷線保留中都算可以認領)，不能
      // 再用token是不是空的來判斷。
      const isAvailable = (s) => {
        const entry = Array.from(room.players.values()).find(p => p.token === s.token);
        return !entry || entry.disconnected;
      };
      // 這裡走到，代表最上面那段「身份代碼直接吻合」的檢查沒有命中——不要
      // 隨便挑「第一個還沒人的座位」自動認領，猜錯的話這個人會頂著別人的
      // 名字繼續玩。改成回報座位名單，讓前端顯示選擇畫面，選好之後呼叫
      // join_room_from_save確認要認領哪一個。
      const available = seatOrder.filter(isAvailable);
      if (available.length === 0) return callback({ ok: false, error: '這場遊戲的座位目前都還在線上，沒有空位可以認回；如果你是原本的玩家之一，代表你原本那個分頁應該還連著，回去那邊繼續就好' });
      return callback({ ok: true, roomCode, needsSeatChoice: true, availableSeats: available.map(s => s.name) });
    }
    if (room.started) {
  ...
  // ---- 房主新增/移除 AI 玩家：AI座位沒有真正的socket連線，用一組固定的假id
  // (ai_1, ai_2...)代表，跟真人玩家一起放進同一個 room.players Map 裡，
  // 這樣後續「開始遊戲」時，AI座位會自然地跟真人玩家一起被算進 seatOrder，
  // 不需要另外處理。實際上場後，AI的回合是由房主的client代打(見前端邏輯)。----
  socket.on('add_ai_player', (data, callback) => {
    const roomCode = socket.data.roomCode;
    const room = rooms.get(roomCode);
    if (!room) return callback && callback({ ok:false, error:'找不到房間' });
    if (room.hostId !== socket.id) return callback && callback({ ok:false, error:'只有房主能新增AI玩家' });
    if (room.started) return callback && callback({ ok:false, error:'遊戲已經開始了' });
    if (room.players.size >= MAX_PLAYERS_PER_ROOM) return callback && callback({ ok:false, error:'房間已滿（最多4人）' });

    const aiCount = Array.from(room.players.values()).filter(p => p.isAI).length;
    const aiId = 'ai_' + (room.aiSeq = (room.aiSeq||0) + 1);
    const token = generateToken();
    room.players.set(aiId, {
      id: aiId, token, name: 'AI玩家' + (aiCount + 1), isHost: false, ready: true,
      characterId: AI_CHAR_POOL[aiCount % AI_CHAR_POOL.length], isAI: true,
    });
    console.log(`[新增AI玩家] ${roomCode} -> ${aiId}`);
    callback && callback({ ok:true });
    broadcastRoomState(roomCode);
  });
  const AI_CHAR_POOL = ['char1', 'char2', 'char3', 'char4',
    'char5', 'char6', 'char7', 'char8', 'char9', 'char10', 'char11', 'char12',
    'char13', 'char14', 'char15', 'char16', 'char17', 'char18', 'char19', 'char20', 'char21'];

  socket.on('remove_ai_player', ({ aiId }, callback) => {
    const roomCode = socket.data.roomCode;
    const room = rooms.get(roomCode);
    if (!room) return callback && callback({ ok:false, error:'找不到房間' });
    if (room.hostId !== socket.id) return callback && callback({ ok:false, error:'只有房主能移除AI玩家' });
    if (room.started) return callback && callback({ ok:false, error:'遊戲已經開始了' });
    const p = room.players.get(aiId);
    if (!p || !p.isAI) return callback && callback({ ok:false, error:'找不到這個AI玩家' });
    room.players.delete(aiId);
    console.log(`[移除AI玩家] ${roomCode} -> ${aiId}`);
    callback && callback({ ok:true });
    broadcastRoomState(roomCode);
  });
  ...
  socket.on('join_as_participant', ({ roomCode, playerToken }, callback) => {
    ...
    // 不要讓它苦等一個早就已經過去的廣播
    if (room.pw) {
      socket.emit('password_game_start', { seatOrder: room.pw.seatOrder, rangeMax: room.pw.rangeMax, startSeatIdx: room.pw.currentTurnSeatIdx, triggerName: room.activeMinigame ? room.activeMinigame.triggerName : null });
    }
    if (room.rl) {
      socket.emit('redlight_game_start', { seatOrder: room.rl.seatOrder, trackLen: room.rl.trackLen, progress: room.rl.progress, caughtTokens: Array.from(room.rl.caughtTokens||[]), triggerName: room.activeMinigame ? room.activeMinigame.triggerName : null });
    ...
    if (room.gt) {
      socket.emit('ghost_tag_game_start', {
        ...
        triggerToken: room.gt.triggerToken, triggerName: room.activeMinigame ? room.activeMinigame.triggerName : null,
      });
    }
    if (room.ps) {
      socket.emit('plane_shooter_game_start', {
        seatOrder: room.ps.seatOrder, matchStartAt: room.ps.matchStartAt, triggerToken: room.ps.triggerToken, triggerName: room.activeMinigame ? room.activeMinigame.triggerName : null,
      });
    }
    if (room.mj && room.mj.hands[playerToken]) {
      socket.emit('mahjong_game_start', {
        ...
        triggerName: room.activeMinigame ? room.activeMinigame.triggerName : null,
      });
    ...
    if (room.siege) {
      if (room.siege.phase === 'tankpick') {
        ...
        socket.emit('siege_match_start', {
          seatOrder: room.siege.seatOrder.map(s => ({ token:s.token, name:s.name, isAI: !!s.isAI, tankKey: room.siege.players[s.token]?.tankKey })),
  ...
  socket.on('start_monopoly_game', ({ rule, startingMoney, turnLimit, enabledMinigames, siegeDifficulty }, callback) => {
    const roomCode = socket.data.roomCode;
    const room = rooms.get(roomCode);
    if (!room) return callback && callback({ ok:false, error:'找不到房間' });
    ...
    const seatOrder = Array.from(room.players.values()).map((p, i) => ({ id: p.id, token: p.token, name: p.name, seatIndex: i, characterId: p.characterId || 'char1', isAI: !!p.isAI }));
    ...
    // 攻城戰難度：簡單/普通/困難 對應到「敵方開火速度」的額外倍率，數字越大代表
    // 額外再拉慢多少倍(困難=1代表不額外拉慢，直接用基礎速度)
    const SIEGE_DIFFICULTY_MULTIPLIERS = { easy: 2, normal: 1, hard: 0.6 };
    const safeSiegeDifficulty = SIEGE_DIFFICULTY_MULTIPLIERS[siegeDifficulty] ? siegeDifficulty : 'normal';
    room.mp = {
      seatOrder,
      ...
      siegeDifficulty: safeSiegeDifficulty,
    };
    room.started = true;
  ...
  socket.on('request_roll', (data, callback) => {
    ...
    // AI玩家沒有自己真正的連線，是由房主的client代打——如果現在輪到的
    // 座位是AI，代打的請求送過來時，用的是房主自己的token，不會等於
    // AI座位自己的token，原本這裡只認seat.token跟自己token相不相等，
    // 導致「房主幫AI擲骰」一律被判定成「還沒輪到你」而擲骰失敗、整個
    // 卡住。這裡補上例外：如果現在這個座位是AI、而且發送請求的人正是
    // 房主本人，就放行。
    const isHostActingForAI = seat && seat.isAI && room.hostId === socket.id;
    if (!seat || (seat.token !== socket.data.playerToken && !isHostActingForAI)) {
      return callback && callback({ ok:false, error:'還沒輪到你' });
    }
  ...
  socket.on('state_sync', ({ snapshot, nextSeatIdx }) => {
    ...
    if (!room || !room.mp) return;
    const seat = room.mp.seatOrder[room.mp.currentTurnSeatIdx];
    // 跟 request_roll 同一個問題：AI玩家是由房主代打，狀態快照送過來時
    // 用的是房主自己的token，原本這裡的檢查會讓AI回合跑完的結果整個送不
    // 出去、也不會換到下一位玩家，遊戲就卡住不動——這其實比擲骰失敗更嚴重，
    // 因為擲骰失敗至少還有錯誤訊息，這裡是靜靜地什麼事都沒發生。同樣補上
    // 房主幫AI代打時的例外。
    const isHostActingForAI = seat && seat.isAI && room.hostId === socket.id;
    if (!seat || (seat.token !== socket.data.playerToken && !isHostActingForAI)) return; // 只信任目前輪到的玩家(或代打的房主)送出的快照
  ...
  // AI選要打出哪張牌：跟client端單機模式同一套邏輯——算每張牌對湊組合的
  // 「有用程度」，留下最有用的，打掉最沒用的那張。
  function mjAiChooseDiscard(hand){
    const counts = mjToCounts(hand);
  ...
  function mjDrawPhase(roomCode){
    ...
    if (mjIsWinningHand(mj.hands[seat.token])) {
      ...
      // AI座位：自摸永遠直接宣告胡牌，不用像真人一樣等12秒逾時才處理，
      // 不然每次AI自摸這局都要多卡12秒。
      mj.pendingTimer = seat.isAI
        ? setTimeout(() => mjResolveSelfWin(roomCode, seat.token, true), 700)
        : setTimeout(() => mjResolveSelfWin(roomCode, seat.token, false), 12000);
    } else {
  ...
  function mjAwaitDiscard(roomCode){
    ...
    clearTimeout(mj.pendingTimer);
    if (seat.isAI) {
      // AI座位：用跟client端單機模式一樣的邏輯算出要打哪張牌，短暫延遲一下
      // 讓畫面上看起來像「思考中」，不用像真人一樣空等20秒逾時才用預設的
      // 「打最後一張」。
      mj.pendingTimer = setTimeout(() => {
        const hand = mj.hands[seat.token];
        if (!hand || !hand.length) return;
        const choice = mjAiChooseDiscard(hand);
        mjResolveDiscard(roomCode, seat.token, choice.id);
      }, 600);
      return;
    }
    mj.pendingTimer = setTimeout(() => {
  ...
  function mjOfferHuChain(roomCode, offset){
    ...
    if (mjIsWinningHand(candidateHand)) {
      ...
      clearTimeout(mj.pendingTimer);
      // AI座位：只要能胡就一定胡，不用像真人一樣等12秒逾時才處理
      mj.pendingTimer = seat.isAI
        ? setTimeout(() => mjResolveHu(roomCode, seat.token, true), 700)
        : setTimeout(() => mjResolveHu(roomCode, seat.token, false), 12000);
    } else {
  ...
  function bjAdvanceTurn(roomCode){
    ...
    if (seat.isAI) {
      // AI座位：用簡單的21點策略(未滿17就要牌，17以上停牌)立刻決定，
      // 不用像真人一樣空等15秒逾時才用預設的「停牌」。
      bj.pendingTimer = setTimeout(() => {
        const total = bjComputeTotal(bj.hands[seat.token]);
        bjResolveAction(roomCode, seat.token, total < 17 ? 'hit' : 'stand');
      }, 700);
      return;
    }
  ...
  function bjResolveAction(roomCode, token, action){
    ...
    if (action === 'hit') {
      ...
      if (total > 21) {
        ...
        if (seat.isAI) {
          bj.pendingTimer = setTimeout(() => {
            const t2 = bjComputeTotal(bj.hands[token]);
            bjResolveAction(roomCode, token, t2 < 17 ? 'hit' : 'stand');
          }, 700);
        } else {
  ...
  function startMinigameInternal(roomCode, gameId){
    ...
    if (gameId === 'ultimate_password') {
      ...
      io.to(roomCode).emit('password_game_start', { seatOrder, rangeMax, startSeatIdx: 0, triggerName: room.activeMinigame.triggerName });
      pwScheduleAiFallback(roomCode);
    }
      ...
      io.to(roomCode).emit('redlight_game_start', { seatOrder, trackLen, progress, triggerName: room.activeMinigame.triggerName });
      ...
      io.to(roomCode).emit('ghost_tag_game_start', { seatOrder, obstacles, itIndex, matchStartAt, triggerToken, triggerName: room.activeMinigame.triggerName });
    }
      ...
      io.to(roomCode).emit('plane_shooter_game_start', { seatOrder, matchStartAt, triggerToken, triggerName: room.activeMinigame.triggerName });
  ...
  socket.on('trigger_minigame', ({ reason, tileIndex }, callback) => {
    ...
    const room = rooms.get(roomCode);
    if (!room || !room.mp) return callback && callback({ ok:false, error:'尚未開局' });
    // 跟 request_roll 同一個問題：這裡原本直接拿發送請求那個socket自己的
    // playerToken當成「觸發者」，如果現在輪到的是AI座位、由房主代打觸發
    // 小遊戲，觸發者身份就會被誤記成房主自己——後面判定「觸發者贏了沒」
    // 會整個對錯人，變成房主(真人)自己贏了小遊戲反而幫AI買到地、蓋到房，
    // 這正是玩家回報的「AI發起的小遊戲，我贏了反而可以買地」。修法跟
        // request_roll一致：觸發者身份要看「現在棋盤輪到誰」，不是看誰送出這個
    // request——真人自己觸發沒問題，AI座位觸發則需要驗證是房主代打送的。
    const seat = room.mp.seatOrder[room.mp.currentTurnSeatIdx];
    const isHostActingForAI = seat && seat.isAI && room.hostId === socket.id;
    if (!seat || (seat.token !== socket.data.playerToken && !isHostActingForAI)) {
      return callback && callback({ ok:false, error:'還沒輪到你' });
    }
    const triggerSeat = seat;
  ...
  // 送出一次終極密碼的猜測，不管是真人親自送出、房主代打AI、還是伺服器自己
  // 的逾時保險機制觸發，都走這同一份邏輯，確保三種來源的行為/廣播完全一致。
  function pwSubmitGuessInternal(roomCode, guess){
    const room = rooms.get(roomCode);
    if (!room || !room.pw) return { ok:false, error:'尚未開局' };
    const pw = room.pw;
    const seat = pw.seatOrder[pw.currentTurnSeatIdx];
    if (!seat) return { ok:false, error:'找不到目前輪到的座位' };
    if (typeof guess !== 'number' || guess < pw.low || guess > pw.high) {
      return { ok:false, error:'猜測超出目前範圍' };
    }
    clearTimeout(pw.aiFallbackTimer);
    ...
    if (bingo) {
      ...
      if (room.activeMinigame && !room.activeMinigame.resultReported) {
        room.activeMinigame.resultReported = true;
        const success = guesserToken === room.activeMinigame.triggerToken;
        io.to(roomCode).emit('minigame_result', {
  ...
  // 保險機制：輪到AI座位時，不管有沒有任何真人client成功代打，伺服器自己
  // 頂多等 2.5 秒，時間到就自己算一個合理的猜測送出去，確保這場小遊戲不會
  // 因為「代打的那個client剛好不是房主/掉線/程式有問題」而永遠卡住。
  function pwScheduleAiFallback(roomCode){
    const room = rooms.get(roomCode);
    ...
    pw.aiFallbackTimer = setTimeout(() => {
      const cur = rooms.get(roomCode);
      if (!cur || !cur.pw) return;
      const curSeat = cur.pw.seatOrder[cur.pw.currentTurnSeatIdx];
      if (!curSeat || curSeat.token !== seatToken) return; // 這期間已經有人送出/換人了，不要送舊的
      const mid = Math.round((cur.pw.low + cur.pw.high) / 2);
      const guess = Math.random() < 0.6 ? mid : Math.floor(cur.pw.low + Math.random()*(cur.pw.high - cur.pw.low + 1));
      pwSubmitGuessInternal(roomCode, guess);
    }, 2500);
  }
  ...
  socket.on('password_submit_guess', ({ guess }, callback) => {
    ...
    const pw = room.pw;
    const seat = pw.seatOrder[pw.currentTurnSeatIdx];
    // 同樣補上房主代打AI座位時的例外，跟大富翁本體回合的修法一致。
    const isHostActingForAI = seat && seat.isAI && room.hostId === socket.id;
    if (!seat || (seat.token !== socket.data.playerToken && !isHostActingForAI)) return callback && callback({ ok:false, error:'還沒輪到你' });
    const result = pwSubmitGuessInternal(roomCode, guess);
  ...
  socket.on('redlight_move', ({ progress, aiToken }) => {
    const roomCode = socket.data.roomCode;
    const room = rooms.get(roomCode);
    if (!room || !room.rl) return;
    let token = socket.data.playerToken;
    // 房主代打AI座位回報進度時的例外，跟鬼抓人那邊的驗證方式一致
    if (aiToken) {
      const isHost = room.hostId === socket.id;
      const targetSeat = (room.rl.seatOrder || []).find(s => s.token === aiToken);
      if (isHost && targetSeat && targetSeat.isAI) {
        token = aiToken;
      } else {
        return;
      }
  ...
  socket.on('redlight_caught', ({ aiToken } = {}) => {
    const roomCode = socket.data.roomCode;
    const room = rooms.get(roomCode);
    if (!room || !room.rl) return;
    let token = socket.data.playerToken;
  ...
  socket.on('redlight_finished', ({ aiToken } = {}, callback) => {
    const roomCode = socket.data.roomCode;
    const room = rooms.get(roomCode);
    ...
    if (aiToken) {
      ...
      if (isHost && targetSeat && targetSeat.isAI) {
        ...
        return callback && callback({ ok:false });
      }
    }
  ...
  socket.on('ghosttag_move', ({ x, z, aiToken }) => {
    const roomCode = socket.data.roomCode;
    const room = rooms.get(roomCode);
    if (!room || !room.gt) return;
    if (typeof x !== 'number' || typeof z !== 'number') return;
    let token = socket.data.playerToken;
    // 房主的client除了回報自己的位置，也會順便幫忙代打AI座位、一併回報
    // AI的位置——這裡收到aiToken時，要先驗證：(1)發送的人真的是這個房間
    // 的房主，(2)aiToken對應的座位真的是AI，兩者都符合才信任這個位置
    // 是屬於那個AI座位，避免有人亂傳假的token冒充別人移動。
    if (aiToken) {
      const isHost = room.hostId === socket.id;
      const targetSeat = (room.gt.seatOrder || []).find(s => s.token === aiToken);
      if (isHost && targetSeat && targetSeat.isAI) {
        token = aiToken;
      } else {
        return; // 驗證不過，整包丟棄，不要用發送者自己的token誤發成AI的位置
      }
    }
  ...
  socket.on('ghosttag_eliminate', ({ targetToken }) => {
    ...
    const isHostActingForAI = itSeat && itSeat.isAI && room.hostId === socket.id;
    if (!itSeat || (itSeat.token !== socket.data.playerToken && !isHostActingForAI)) return;
    io.to(roomCode).emit('ghosttag_eliminate', { targetToken });
  });

  // 「觸發式強制參加」共用的自我回報結果管道：只信任目前這場小遊戲的觸發者自己回報
  socket.on('report_trigger_outcome', ({ success, aiToken }) => {
    const roomCode = socket.data.roomCode;
    const room = rooms.get(roomCode);
    if (!room || !room.activeMinigame || room.activeMinigame.resultReported) return;
    let token = socket.data.playerToken;
    // 觸發這場鬼抓人挑戰的如果是AI座位，沒有任何真人client的token會等於
    // triggerToken，原本這裡的驗證會讓這場挑戰永遠回報不了、卡住不結束。
    // 補上房主代打AI觸發者回報結果的例外。
    if (aiToken) {
      const isHost = room.hostId === socket.id;
      const targetSeat = (room.gt && room.gt.seatOrder || []).find(s => s.token === aiToken);
      if (isHost && targetSeat && targetSeat.isAI) {
        token = aiToken;
      } else {
        return;
    ...
    if (token !== room.activeMinigame.triggerToken) return;
    room.activeMinigame.resultReported = true;
    const triggerSeat = (room.gt && room.gt.seatOrder.find(s => s.token === token)) || {};
    io.to(roomCode).emit('minigame_result', {
      winnerToken: token,
  ...
  socket.on('planeshooter_move', ({ x, y, hp, score, alive, aiToken }) => {
    const roomCode = socket.data.roomCode;
    const room = rooms.get(roomCode);
    if (!room || !room.ps) return;
    let token = socket.data.playerToken;
    // 房主代打AI座位回報血量/分數時的例外，跟其他小遊戲的驗證方式一致
    if (aiToken) {
      const isHost = room.hostId === socket.id;
      const targetSeat = (room.ps.seatOrder || []).find(s => s.token === aiToken);
      if (isHost && targetSeat && targetSeat.isAI) {
        token = aiToken;
      } else {
        return;
  ...
  // ============================================================
  socket.on('blackjack_action', ({ action, aiToken }) => {
    const roomCode = socket.data.roomCode;
    if (!roomCode) return;
    const room = rooms.get(roomCode);
    if (!room || !room.bj) return;
    let token = socket.data.playerToken;
    // 房主代打AI座位要牌/停牌時的例外，跟其他小遊戲的驗證方式一致；
    // 修掉原本這裡的問題：之前雖然有檢查isHost，但實際操作的還是host
    // 自己的token，等於指令送對了、但改到的是host自己的手牌，不是AI的。
    if (aiToken) {
      const isHost = room.hostId === socket.id;
      const targetSeat = (room.bj.seatOrder || []).find(s => s.token === aiToken);
      if (isHost && targetSeat && targetSeat.isAI) {
        token = aiToken;
      } else {
        return;
    ...
    bjResolveAction(roomCode, token, action === 'hit' ? 'hit' : 'stand');
  });
  ...
  const SIEGE_TANK_DEFS = {
    tank_light:  { attack:40,  hp:20000, fireInterval:0.333 },
    tank_medium: { attack:70,  hp:25600, fireInterval:0.25 },
    tank_heavy:  { attack:100, hp:32000, fireInterval:0.2 },
  };
  // 之前24座砲台都用檔案裡寫死的開火間隔，從沒實際拿真實血量+真實人數試算過
  // 整體傷害量——算出來連滿房4人，平均每人每秒要扛住超過1000點傷害，中型戰車
  // 撐不到3秒就陣亡，玩家會覺得「莫名死很快」完全合理。這裡加兩個係數：
  // SIEGE_DIFFICULTY_SLOWDOWN 把整體開火頻率拉慢，SIEGE_REFERENCE_PLAYERS 讓
  // 「每個玩家平均承受的總傷害量」不會因為場上人數多寡而差太多(人少時每座砲台
  // 也跟著打得比較慢)。
  const SIEGE_DIFFICULTY_SLOWDOWN = 8;
  const SIEGE_REFERENCE_PLAYERS = 4;
  // 每座砲台在城堡上的世界座標(x,z)，跟前端 siege_game.html 的 TURRET_DEFS.
  // fixedPositions 完全對應(城堡本體固定在原點、沒有位移/旋轉，所以座標可以
  // 直接照抄，不用另外做座標轉換)——伺服器要能真的判定「這一砲有沒有打中」，
  // 勢必要知道砲台實際在哪裡，不能再像原本那樣完全不管位置、純粹用機率決定
  // 打不打得中。shellSpeed 同樣照抄前端的飛行速度設定，這樣才能正確算出
  // 「這一砲飛到目標位置要花多久」。
  const SIEGE_TURRET_DEFS = [
    { key:'turret_missile', attack:100, maxHp:50000, count:3, fireInterval:0.333, shellSpeed:11, positions:[
      {x:-5.95,z:-2.59},{x:5.93,z:-5.31},{x:-5.88,z:-5.10}
    ] },
    { key:'turret_gatling', attack:80,  maxHp:35000, count:4, fireInterval:0.2, shellSpeed:15, positions:[
      {x:-5.77,z:5.46},{x:6.16,z:-0.03},{x:-5.93,z:0.23},{x:5.42,z:5.39}
    ] },
    { key:'turret_twin',    attack:60,  maxHp:25000, count:5, fireInterval:0.25, shellSpeed:13, positions:[
      {x:-8.99,z:3.85},{x:-8.80,z:-3.71},{x:2.19,z:-2.58},{x:10.60,z:0.35},{x:-0.41,z:8.81}
    ] },
    { key:'turret_flame',   attack:40,  maxHp:17500, count:6, fireInterval:0.286, shellSpeed:10, positions:[
      {x:-8.48,z:-7.19},{x:-1.51,z:-7.79},{x:1.81,z:-7.81},{x:7.97,z:-7.78},{x:8.00,z:7.65},{x:-7.81,z:7.47}
    ] },
    { key:'turret_basic',   attack:20,  maxHp:10000, count:6, fireInterval:0.333, shellSpeed:12, positions:[
      {x:-8.61,z:0.03},{x:5.12,z:-8.07},{x:-5.01,z:-7.78},{x:9.20,z:4.32},{x:2.50,z:8.18},{x:-2.32,z:7.91}
    ] },
  ];
  // 砲彈飛到目標身邊多近算「命中」——跟前端畫面判定砲彈到站的距離(1.4)同一個
  // 概念，另外抓一點寬容值給戰車本身的體積。
  const SIEGE_HIT_RADIUS = 1.6;
  ...
  function siegeScheduleTurretFire(roomCode, turretId){
    ...
    // 原本24座砲台全部用檔案裡寫死的間隔各自開火，從來沒有真正拿實際血量、
    // 實際人數去試算過整體傷害量——算出來就算滿房4人，每人平均每秒要扛住
    // 超過1000點傷害，中型戰車撐不到3秒就陣亡，難怪玩家會覺得「莫名死很快」。
    // 這裡加上：(1) 整體開火頻率統一拉慢(SIEGE_DIFFICULTY_SLOWDOWN)
    // (2) 依照目前存活人數動態調整——人少的時候每座砲台开火頻率也跟著變慢，
    // 讓「每個玩家平均承受的總傷害量」不會因為人數多寡而差太多。
    const aliveCount = Math.max(1, siegeAliveTokens(room).length);
    const SIEGE_DIFFICULTY_MULTIPLIERS = { easy: 2, normal: 1, hard: 0.6 };
    const diffMult = SIEGE_DIFFICULTY_MULTIPLIERS[room.mp && room.mp.siegeDifficulty] || 1;
    const effectiveInterval = def.fireInterval * SIEGE_DIFFICULTY_SLOWDOWN * diffMult * (SIEGE_REFERENCE_PLAYERS / aliveCount);
    turret.fireTimer = setTimeout(() => {
      ...
      if (alive.length > 0) {
        ...
        const p = room2.siege.players[targetToken];
        if (p) {
          // 瞄準的是「開火這一刻」玩家實際站的座標，跟前端畫面的砲彈瞄準點
          // 保持一致。這一砲飛過去要花多久，就用距離/砲彈速度算出來。
          const aimX = p.x, aimZ = p.z;
          const dist = Math.hypot(aimX - t2.x, aimZ - t2.z) || 1;
          const flightMs = Math.max(50, (dist / def.shellSpeed) * 1000);
          // 先廣播「開火了」，讓所有人的畫面立刻開始播放砲彈飛過去的動畫(瞄準
          // 這一刻玩家所在的位置)。真正扣不扣血、扣多少，要等這顆砲彈的飛行
          // 時間真的過完，拿玩家「那個時候」實際站的位置去判斷有沒有真的閃開
          // ——不能像原本那樣一開火就直接扣血，那樣不管玩家怎麼移動都躲不掉，
          // 移動變成純粹裝飾、完全不影響會不會中彈，這正是玩家反映「明明一直
          // 橫向移動、砲彈也沒看到打中，血量卻還是一直掉」的真正根因：伺服器
          // 從頭到尾根本沒有拿玩家的位置去判定過命中，只是機率抽籤決定誰倒楣。
          io.to(roomCode).emit('siege_turret_fired', { turretId, targetToken, damage: def.attack, aimX, aimZ });
          setTimeout(() => {
            const room3 = rooms.get(roomCode);
            if (!room3 || !room3.siege || room3.siege.ended) return;
            const p3 = room3.siege.players[targetToken];
            if (!p3 || !p3.alive) return;
            const hitDist = Math.hypot(p3.x - aimX, p3.z - aimZ);
            if (hitDist > SIEGE_HIT_RADIUS) return; // 這段時間玩家已經移動離開瞄準點，算閃過去了，不扣血
            const p4 = siegeApplyDamageToPlayer(roomCode, targetToken, def.attack);
            if (p4) {
              io.to(roomCode).emit('siege_turret_hit_result', {
                targetToken, hp: p4.hp, alive: p4.alive, lives: p4.lives, permanentlyDead: p4.permanentlyDead,
              });
              if (p4.permanentlyDead) siegeCheckTeamWipe(roomCode);
            }
          }, flightMs);
        }
      }
  ...
  function siegeStartMatch(roomCode){
    ...
    room.siege.players = {};
    seatOrder.forEach((s, i) => {
      const tankKey = room.siege.tankPicks[s.token] || 'tank_medium';
      const def = SIEGE_TANK_DEFS[tankKey] || SIEGE_TANK_DEFS.tank_medium;
      // 跟前端 setupPlayers() 的出生點公式一致(半徑17、依座位順序平均分散角度)，
      // 這樣伺服器一開始就知道每個人大概站在哪，不用等第一次 siege_move 才有
      // 座標可用。之後每次收到 siege_move 都會即時更新成最新位置(見下面
      // socket.on('siege_move', ...))。
      ...
      room.siege.players[s.token] = {
        ...
        x: Math.cos(angle0) * 17, z: Math.sin(angle0) * 17,
      };
    });
    ...
    SIEGE_TURRET_DEFS.forEach(def => {
      for (let i = 0; i < def.count; i++) {
        const pos = def.positions[i] || { x: 0, z: 0 };
        room.siege.turrets.push({ id: turretId++, key: def.key, hp: def.maxHp, maxHp: def.maxHp, alive: true, fireTimer: null, x: pos.x, z: pos.z });
      }
    });
    ...
    io.to(roomCode).emit('siege_match_start', {
      seatOrder: seatOrder.map(s => ({ token: s.token, name: s.name, isAI: !!s.isAI, tankKey: room.siege.players[s.token].tankKey })),
      matchStartAt: room.siege.matchStartAt,
      pot: room.siege.pot,
      castleHp: room.siege.castleHp,
      castleMaxHp: room.siege.castleMaxHp,
  ...
  socket.on('start_siege_game', (data, callback) => {
    ...
    const seatOrder = Array.from(room.players.values()).map(p => ({ id: p.id, token: p.token, name: p.name, isAI: !!p.isAI }));
    room.siege = { phase:'tankpick', seatOrder, tankPicks:{}, readySet:new Set(), players:{}, turrets:[], ended:false };
    // AI座位沒有真人client會幫忙點「選戰車」「準備完成」，原本這裡會讓
    // readySet永遠湊不滿座位數，真人選完戰車按準備之後就卡住不會開打。
    // AI座位直接在這裡當場給預設戰車、標記準備完成，不用等誰代打。
    seatOrder.forEach(s => {
      if (s.isAI) {
        room.siege.tankPicks[s.token] = 'tank_medium';
        room.siege.readySet.add(s.token);
      }
    });
  ...
  // ---- 玩家主動關閉攻城戰畫面：如果整場攻城戰裡只剩自己一個真人(其他都是
  // AI隊友，本來就是自己的client代打，一關掉畫面AI也跟著沒人操作了)，直接把
  // 這場攻城戰結束掉，不要留著在背景一直跑——不然沒人看著它，卻因為
  // room.siege還存在，之後想再挑戰一次會被「攻城戰已經在進行中」擋下來，
  // 得乾等到30分鐘時限自動結束才行。如果還有其他真人隊友仍在攻城戰裡，
  // 就不受影響，維持原本「自己離開畫面、隊友可以繼續玩」的行為。
  socket.on('siege_abandon', (data, callback) => {
    const roomCode = socket.data.roomCode;
    const room = rooms.get(roomCode);
    if (!room || !room.siege) return callback && callback({ ok:true });
    const humanSeats = (room.siege.seatOrder || []).filter(s => !s.isAI);
    if (humanSeats.length <= 1) {
      if (room.siege.phase === 'match' && !room.siege.ended) {
        siegeEndMatch(roomCode, 'abandoned');
      } else if (room.siege.phase === 'tankpick') {
        // 都還沒真的開打(還在選戰車的大廳階段)，沒有結算可言，直接關掉即可
        siegeClearAllTimers(room);
        room.siege = null;
        io.to(roomCode).emit('siege_player_left', { token: socket.data.playerToken, cancelled: true });
      }
    }
    callback && callback({ ok:true });
  });
  ...
  socket.on('siege_move', ({ x, z, rotY, aiToken }) => {
    const roomCode = socket.data.roomCode;
    const room = rooms.get(roomCode);
    ...
    if (aiToken) {
      const isHost = room.hostId === socket.id;
      const targetSeat = (room.siege.seatOrder || []).find(s => s.token === aiToken);
      if (isHost && targetSeat && targetSeat.isAI) { token = aiToken; } else { return; }
    }
    // 記住這個人現在實際站在哪——原本這裡完全沒有存，伺服器對玩家位置一無
    // 所知，砲台開火只能用機率亂射，不管玩家怎麼跑都閃不掉。有記住位置之後，
    // 下面 siegeScheduleTurretFire 才能真的判定「這一砲有沒有打中移動中的你」。
    const p = room.siege.players[token];
    if (p) { p.x = x; p.z = z; }
    socket.to(roomCode).emit('siege_move', { token, x, z, rotY });
  });

  // 玩家戰車開火：純粹轉發給同房間其他人，讓他們的畫面也能顯示這顆子彈的
  // 飛行動畫——傷害判定完全交給開火那個人自己的 siege_hit_turret /
  // siege_hit_castle 那兩個事件處理，這裡不重複計算、也不驗證傷害數字。
  socket.on('siege_player_fire', ({ originX, originZ, targetX, targetY, targetZ, aiToken }) => {
    const roomCode = socket.data.roomCode;
    const room = rooms.get(roomCode);
    if (!room || !room.siege || room.siege.phase !== 'match') return;
    if ([originX, originZ, targetX, targetY, targetZ].some(v => typeof v !== 'number')) return;
    let token = socket.data.playerToken;
    if (aiToken) {
      const isHost = room.hostId === socket.id;
      const targetSeat = (room.siege.seatOrder || []).find(s => s.token === aiToken);
    ...
    socket.to(roomCode).emit('siege_player_fire', { token, originX, originZ, targetX, targetY, targetZ });
  });

  socket.on('siege_hit_castle', ({ amount, aiToken }) => {
    const roomCode = socket.data.roomCode;
    const room = rooms.get(roomCode);
  ...
  socket.on('siege_hit_turret', ({ turretId, amount, aiToken }) => {
    const roomCode = socket.data.roomCode;
    const room = rooms.get(roomCode);
    if (!room || !room.siege || room.siege.phase !== 'match' || room.siege.ended) return;
    let token = socket.data.playerToken;
  ...
  socket.on('save_game_progress', ({ note }, callback) => {
    ...
    const doc = {
      ...
      roomCode, // 記住原本的房號——讀檔時要優先嘗試回到「同一個房間」，而不是每次都開一個全新、
                // 跟原本無關的房間，不然其他原本在玩的人根本不知道要去哪裡才找得到大家
  ...
  socket.on('list_saves', (data, callback) => {
    ...
    db.collection('saves').find({}).sort({ savedAt:-1 }).limit(20).toArray()
      .then(docs => callback && callback({ ok:true, saves: docs.map(d => ({
        id: d._id.toString(), savedAt: d.savedAt, savedBy: d.savedBy, note: d.note, roomCode: d.roomCode || null,
        players: d.seatOrder.map(s => s.name), rule: d.rule,
      })) }))
      .catch(err => callback && callback({ ok:false, error:err.message }));
  });

  // 從存檔開一個房間。優先嘗試「回到原本那個房號」，而不是每次都開一個全新、
  // 跟原本無關的房間——不然其他原本一起玩的人，根本不知道要去哪裡才找得到大家，
  // 「複製房號」這種要手動傳出去的做法，並不會真的解決這個協調問題。
  socket.on('create_room_from_save', ({ saveId, playerName, playerToken }, callback) => {
    if (!dbReady) return callback && callback({ ok:false, error:'存檔功能目前未啟用（尚未設定資料庫）' });
    db.collection('saves').findOne({ _id: new ObjectId(saveId) })
      .then(doc => {
        if (!doc) return callback && callback({ ok:false, error:'找不到這筆存檔' });

        // 這筆存檔記錄的原始房號，如果現在剛好還活著(代表已經有人剛剛讀過這筆存檔、
        // 房間還沒過期或結束)，不要再另外開一個無關的新房間——請這個人改用一般的
        // 「加入房間」、直接輸入這個房號，會自動比對身份代碼，是原本這個座位的人
        // 就直接接回去，不是的話就當一般新加入處理。
        if (doc.roomCode && rooms.has(doc.roomCode)) {
          return callback && callback({
            ok:false, alreadyLive:true, roomCode:doc.roomCode,
            error:'這個房間目前還在進行中，改用「加入房間」輸入房號 '+doc.roomCode+' 即可回到原本的遊戲',
          });
        }

        // 房號沒有活著(可能還沒有人讀過這筆存檔，或伺服器重開過)：用「原本的房號」
        // 重新開房，不要另外生一個無關的新房號，這樣之後不管是誰、不管隔多久再讀
        // 同一筆存檔，都會透過上面那個「房號還活著」的判斷，正確走到同一個房間。
        const roomCode = doc.roomCode || generateRoomCode();
        // 保留每個座位原本的身份代碼(不清空)——原本的玩家如果本機還留著這組代碼
        // (例如只是重新整理頁面、沒有特地按過「退出遊戲」)，用「加入房間」就能直接
        // 無縫接回去，不用另外走這條讀檔認領流程。
        const seatOrder = doc.seatOrder.map(s => ({ ...s }));
        const room = { players: new Map(), hostId: null, createdAt: Date.now(), started: true };
        room.mp = { seatOrder, currentTurnSeatIdx: doc.currentTurnSeatIdx, rule: doc.rule, lastSnapshot: doc.snapshot, loadedFromSave: true };
        rooms.set(roomCode, room);

        // 每個座位先當成「保留中、還沒有真人連上來」的狀態(用假的id占位、標記
        // disconnected)，等真正的玩家帶著吻合的身份代碼用「加入房間」回來，就會被
        // join_room那邊既有的認回邏輯接手、換成真正的socket id——直接沿用同一套
        // 認回機制，不用另外重寫一次。保留時間比一般斷線久(1小時)，因為玩家們可能
        // 得先互相協調一下才會陸續回來。
        seatOrder.forEach((s, i) => {
          // AI座位沒有真正玩家會「回來認領」——跟新增AI玩家(add_ai_player)一樣，
          // 直接當成已連線的AI玩家放進room.players，不要也走下面這套「斷線保留中、
          // 等人認領」的邏輯，不然 mpCheckAllSeatsClaimedFromSave 會永遠等不到AI
          // 被認領（它的disconnected永遠是true），讀檔開的房間就會卡在「解析角色
          // 資料中…」動彈不得，遊戲永遠不會正式開始。
          if (s.isAI) {
            room.players.set(s.id, {
              id: s.id, token: s.token, name: s.name, isHost: i === 0, ready: true,
              characterId: null, isAI: true,
            });
            if (i === 0) room.hostId = s.id;
            return;
          }
          const placeholderId = 'presave_' + s.token;
          room.players.set(placeholderId, {
            id: placeholderId, token: s.token, name: s.name, isHost: i === 0, ready: false, characterId: null,
            disconnected: true, disconnectedAt: Date.now(),
            graceTimer: setTimeout(() => finalizeDisconnect(roomCode, placeholderId), LOADED_SAVE_GRACE_MS),
          });
          if (i === 0) room.hostId = placeholderId;
        });

        console.log(`[讀檔開房] ${roomCode} <- 存檔 ${saveId}`);

        // 開房的這個人自己也順便直接認領一個座位，不用另外再呼叫一次：
        // 有帶身份代碼且吻合的話，直接是原本那個座位；沒有吻合的話，退而求其次
        // 用名字比對(playerName跟存檔裡的座位名稱剛好一樣的話)。兩個都對不上
        // 時，不要用「第一個還沒人的座位」隨便猜——猜錯的話這個人就會頂著別人
        // 的名字繼續玩，資料會兜不起來。改成回報「請你自己選你當時是誰」，把
        // 名字清單交給前端顯示選擇畫面，選好之後再呼叫join_room_from_save確認。
        let claimSeat = null;
        if (playerToken) claimSeat = seatOrder.find(s => s.token === playerToken);
        if (!claimSeat && playerName) claimSeat = seatOrder.find(s => s.name === playerName);

        if (!claimSeat) {
          callback && callback({ ok:true, roomCode, needsSeatChoice:true, availableSeats: seatOrder.map(s=>s.name) });
          return;
        }

        const placeholderId = 'presave_' + claimSeat.token;
        const oldEntry = room.players.get(placeholderId);
        clearTimeout(oldEntry && oldEntry.graceTimer);
        room.players.delete(placeholderId);
        const wasHost = room.hostId === placeholderId;
        const newToken = generateToken();
        room.players.set(socket.id, { id:socket.id, token:newToken, name:claimSeat.name, isHost:wasHost, ready:false, characterId:null });
        if (wasHost) room.hostId = socket.id;
        claimSeat.token = newToken;
        claimSeat.id = socket.id;

        socket.join(roomCode);
        socket.data.roomCode = roomCode;
        socket.data.playerToken = newToken;

        const remainingSeats = seatOrder.filter(s => s.token !== newToken).map(s => s.name);
        callback && callback({ ok:true, roomCode, playerToken:newToken, yourSeatName:claimSeat.name, remainingSeats });
        broadcastRoomState(roomCode);
        mpCheckAllSeatsClaimedFromSave(roomCode);
      })
      .catch(err => callback && callback({ ok:false, error:err.message }));
  ...
  // 原本其他玩家，用同一個房號、指定自己是哪個座位名稱來認回——用在「本機沒有
  // 留著吻合的身份代碼」的情況(例如換了裝置、清過本機資料、或從開場畫面重新
  // 讀檔進來)，不然一般應該是透過 join_room 自動比對代碼接回去就好，不用特地
  // 呼叫這個。原本這裡限定「只有讀檔開的房才能用」，但join_room那邊現在只要
  // 是任何已經開局的房、代碼對不上都會導來這裡，所以這裡也要跟著放寬，不然
  // join_room轉過來的請求反而會被這裡用「找不到這個讀檔房間」擋下來。
  socket.on('join_room_from_save', ({ roomCode, seatName }, callback) => {
    roomCode = (roomCode || '').toUpperCase().trim();
    const room = rooms.get(roomCode);
    if (!room || !room.mp) return callback && callback({ ok:false, error:'找不到這個房間，或這個房間目前沒有進行中的遊戲' });
    const seatOrder = room.mp.seatOrder;
    // 一個座位算「還可以被認領」：目前沒有任何連線(不管是真的在線上、還是斷線
    // 保留中)代表它的身份代碼——用連線狀態動態判斷，而不是看token是不是空的，
    // 因為現在座位一開始就帶著存檔當時的原始token，不會是空的。
    const isAvailable = (s) => {
      const entry = Array.from(room.players.values()).find(p => p.token === s.token);
      return !entry || entry.disconnected;
    };
    const nextSeat = seatName ? seatOrder.find(s => s.name === seatName && isAvailable(s)) : seatOrder.find(isAvailable);
    if (!nextSeat) return callback && callback({ ok:false, error: seatName ? '這個座位已經有人認領了' : '這場遊戲的座位目前都還在線上，沒有空位可以認回', availableSeats: seatOrder.filter(isAvailable).map(s=>s.name) });

    // 找出這個座位原本掛著的舊連線紀錄，不管它是讀檔房的假id佔位(presave_開頭)、
    // 還是一般正常斷線後保留在room.players裡的舊socket id，一律用token去比對，
    // 不要只假設一定是presave_這個固定格式——不然一般房間(不是讀檔房)走到這裡
    // 時，舊的斷線紀錄會因為key對不上而永遠留著沒清掉，變成殘留的重複座位。
    const oldEntryPair = Array.from(room.players.entries()).find(([, p]) => p.token === nextSeat.token);
    const oldEntryId = oldEntryPair ? oldEntryPair[0] : null;
    const oldEntry = oldEntryPair ? oldEntryPair[1] : null;
    if (oldEntry) { clearTimeout(oldEntry.graceTimer); room.players.delete(oldEntryId); }
    const wasHost = room.hostId === oldEntryId;

    const token = generateToken();
    nextSeat.token = token;
    nextSeat.id = socket.id;
    room.players.set(socket.id, { id:socket.id, token, name:nextSeat.name, isHost:wasHost, ready:false, characterId:null });
    if (wasHost) room.hostId = socket.id;
    socket.join(roomCode);
    socket.data.roomCode = roomCode;
    socket.data.playerToken = token;
    ...
    const stillWaiting = seatOrder.filter(isAvailable).map(s => s.name);
    const payload = { ok:true, roomCode, playerToken:token, yourSeatName:nextSeat.name, remainingSeats: stillWaiting, state: getPublicRoomState(room) };
    // 這個房間如果根本不是「讀檔重開、大家要陸續回來、全部到齊才正式開局」的
    // 情境（room.mp.loadedFromSave是false）——代表遊戲本來就還活著、一直在
    // 進行中，其他人根本沒有離開過，沒有誰要等誰。這種情況認回座位後要直接
    // 把目前進行到一半的完整狀態給這個人，讓他直接接回棋盤，不能還是走「等待
    // 其他人陸續認領」那一套，不然這個人的畫面會卡在等待畫面等到天荒地老
    // （因為mpCheckAllSeatsClaimedFromSave只有loadedFromSave的房間才會生效）。
    if (!room.mp.loadedFromSave) {
      payload.mpStarted = true;
      payload.mpGameStart = { seatOrder: room.mp.seatOrder, rule: room.mp.rule, startSeatIdx: room.mp.currentTurnSeatIdx };
      payload.mpSnapshot = room.mp.lastSnapshot || null;
      payload.mpCurrentTurnSeatIdx = room.mp.currentTurnSeatIdx;
    }
  ...
  // 讀檔房間專用：檢查是不是每個座位都已經有真人連著了(不是斷線保留中)，
  // 是的話才正式廣播「大家都回來了，開始吧」。create_room_from_save、
  // join_room_from_save、以及join_room裡的身份代碼自動認回，三個地方
  // 只要有人成功認領座位，都要呼叫這個檢查一次。
  function mpCheckAllSeatsClaimedFromSave(roomCode) {
    const room = rooms.get(roomCode);
    if (!room || !room.mp || !room.mp.loadedFromSave) return;
    const seatOrder = room.mp.seatOrder;
    const allClaimed = seatOrder.every(s => {
      const entry = Array.from(room.players.values()).find(p => p.token === s.token);
      return entry && !entry.disconnected;
    });
    if (allClaimed) mpBroadcastLoadedGameStart(roomCode);
  }
  ...
  // ============================================================
  // 棋盤上發起攻城戰：邀請房間所有人，10秒內回應同意/拒絕，
  // 逾時未回應視為自動加入；只要有任何一人明確拒絕，整個提案取消。
  // ============================================================
  const SIEGE_PROPOSAL_TIMEOUT_MS = 10000;

  socket.on('propose_siege', (data, callback) => {
    const roomCode = socket.data.roomCode;
    const room = rooms.get(roomCode);
    if (!room || !room.mp) return callback && callback({ ok:false, error:'尚未開局' });
    if (room.siege) return callback && callback({ ok:false, error:'攻城戰已經在進行中' });
    if (room.siegeProposal) return callback && callback({ ok:false, error:'已經有人正在發起攻城戰邀請，請稍候' });

    const proposer = room.players.get(socket.id);
    if (!proposer) return callback && callback({ ok:false, error:'找不到你的座位' });
    const activeMembers = Array.from(room.players.values()).filter(p => !p.disconnected);
    if (activeMembers.length < 2) return callback && callback({ ok:false, error:'至少需要2位在線玩家才能發起攻城戰' });

    const responses = {};
    activeMembers.forEach(p => { responses[p.token] = (p.token === proposer.token) ? 'accept' : null; });
    room.siegeProposal = { proposerToken: proposer.token, responses };
    room.siegeProposal.timer = setTimeout(() => resolveSiegeProposal(roomCode), SIEGE_PROPOSAL_TIMEOUT_MS);

    io.to(roomCode).emit('siege_proposed', { proposerName: proposer.name, timeoutMs: SIEGE_PROPOSAL_TIMEOUT_MS });
    console.log(`[攻城戰邀請發起] ${roomCode} by ${proposer.name}`);
    callback && callback({ ok:true });
  });

  socket.on('respond_siege_proposal', ({ accept }, callback) => {
    const roomCode = socket.data.roomCode;
    const room = rooms.get(roomCode);
    if (!room || !room.siegeProposal) return callback && callback({ ok:false, error:'邀請已經結束了' });
    const token = socket.data.playerToken;
    if (!(token in room.siegeProposal.responses)) return callback && callback({ ok:false, error:'你不在這次的邀請名單裡' });

    room.siegeProposal.responses[token] = accept ? 'accept' : 'decline';
    callback && callback({ ok:true });

    if (!accept) {
      // 只要有人明確拒絕，不用等其他人，立刻取消整個提案
      clearTimeout(room.siegeProposal.timer);
      room.siegeProposal = null;
      io.to(roomCode).emit('siege_proposal_cancelled', { reason:'declined' });
      console.log(`[攻城戰邀請被拒絕取消] ${roomCode}`);
      return;
    }
    const allResponded = Object.values(room.siegeProposal.responses).every(r => r !== null);
    if (allResponded) resolveSiegeProposal(roomCode);
  });

  function resolveSiegeProposal(roomCode) {
    const room = rooms.get(roomCode);
    if (!room || !room.siegeProposal) return;
    clearTimeout(room.siegeProposal.timer);
    const responses = room.siegeProposal.responses;
    // 逾時沒回應的人，依規格視為自動加入
    Object.keys(responses).forEach(t => { if (responses[t] === null) responses[t] = 'accept'; });
    const declined = Object.values(responses).some(r => r === 'decline');
    room.siegeProposal = null;
    ...
    // 全部同意（含逾時自動加入）：正式開啟攻城戰大廳，沿用跟 start_siege_game 一樣的資料結構
    const seatOrder = Array.from(room.players.values()).filter(p => !p.disconnected)
      .map(p => ({ id: p.id, token: p.token, name: p.name, isAI: !!p.isAI }));
    room.siege = { phase:'tankpick', seatOrder, tankPicks:{}, readySet:new Set(), players:{}, turrets:[], ended:false };
    // 同上：AI座位直接給預設戰車、標記準備完成
    seatOrder.forEach(s => {
      if (s.isAI) {
        room.siege.tankPicks[s.token] = 'tank_medium';
        room.siege.readySet.add(s.token);
    ...
    io.to(roomCode).emit('siege_proposal_accepted', {}); // 通知client開啟攻城戰畫面
    io.to(roomCode).emit('siege_lobby_start', { seatOrder });
    io.to(roomCode).emit('siege_picks_update', { tankPicks: room.siege.tankPicks });
    io.to(roomCode).emit('siege_ready_update', { readyTokens: Array.from(room.siege.readySet) });
    console.log(`[攻城戰邀請全數同意，開局] ${roomCode}`);
  }
