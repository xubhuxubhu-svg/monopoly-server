...
io.on('connection', (socket) => {
  ...
  function bjResolveAction(roomCode, token, action, isHostOverride){
    const room = rooms.get(roomCode);
    if (!room || !room.bj || room.bj.finished) return;
    const bj = room.bj;
    const seat = bj.seatOrder[bj.currentIndex];
    // 跟大富翁本體回合的request_roll/state_sync同一個問題：AI玩家是由
    // 房主代打，這裡補上房主代打AI座位時的例外，否則AI在21點小遊戲裡
    // 要牌/停牌會整個沒有反應。
    const isHostActingForAI = isHostOverride && seat && seat.isAI;
    if (!seat || (seat.token !== token && !isHostActingForAI)) return;
    if (bj.pendingToken !== token && !isHostActingForAI) return;
    clearTimeout(bj.pendingTimer);
  ...
  socket.on('blackjack_action', ({ action }) => {
    ...
    bjResolveAction(roomCode, socket.data.playerToken, action === 'hit' ? 'hit' : 'stand', isHost);
  });
