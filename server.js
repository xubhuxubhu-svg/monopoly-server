...
io.on('connection', (socket) => {
  ...
  socket.on('join_room', ({ roomCode, playerName, playerToken }, callback) => {
    ...
    if (room.mp && room.mp.loadedFromSave) {
      ...
      if (!nextSeat) return callback({ ok: false, error: '這筆存檔的座位都已經有人認領了' });
      ...
      socket.data.roomCode = roomCode;
      socket.data.playerToken = token;
      console.log(`[讀檔認領座位，經由一般加入] ${roomCode} -> ${nextSeat.name}`);
      broadcastRoomState(roomCode);
      const stillWaiting = seatOrder.filter(s => !s.token).map(s => s.name);
      callback({ ok: true, roomCode, playerToken: token, state: getPublicRoomState(room), loadedFromSaveSeatName: nextSeat.name });
  ...
  socket.on('siege_player_fire', ({ originX, originZ, targetX, targetY, targetZ }) => {
    const roomCode = socket.data.roomCode;
    const room = rooms.get(roomCode);
    if (!room || !room.siege || room.siege.phase !== 'match') return;
    if ([originX, originZ, targetX, targetY, targetZ].some(v => typeof v !== 'number')) return;
    socket.to(roomCode).emit('siege_player_fire', { token: socket.data.playerToken, originX, originZ, targetX, targetY, targetZ });
  });
