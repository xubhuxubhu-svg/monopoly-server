...
io.on('connection', (socket) => {
  ...
  socket.on('join_room', ({ roomCode, playerName, playerToken }, callback) => {
    ...
    // 這個房間如果是「讀檔」開的房，原本的其他玩家用一般加入房間的方式進來
    // (而不是走 join_room_from_save)——之前這裡會被下面「room.started」那條擋下來，
    // 誤判成「遊戲已經開始了」，導致玩家2永遠加入不了讀檔房間。這裡先檢查是不是
    // 讀檔房，是的話導去正確的認領座位流程，而不是直接擋掉。
    if (room.mp && room.mp.loadedFromSave) {
      ...
      if (available.length === 0) return callback({ ok: false, error: '這筆存檔的座位都已經有人認領了' });
  ...
  // 留著吻合的身份代碼」的情況(例如換了裝置、或清過本機資料)，不然一般應該是
  // 透過 join_room 自動比對代碼接回去就好，不用特地呼叫這個。
  socket.on('join_room_from_save', ({ roomCode, seatName }, callback) => {
    ...
    if (!nextSeat) return callback && callback({ ok:false, error: seatName ? '這個座位已經有人認領了' : '這筆存檔的座位都已經有人認領了', availableSeats: seatOrder.filter(isAvailable).map(s=>s.name) });

    const oldPlaceholderId = 'presave_' + nextSeat.token;
    const oldEntry = room.players.get(oldPlaceholderId);
    if (oldEntry) { clearTimeout(oldEntry.graceTimer); room.players.delete(oldPlaceholderId); }
    const wasHost = room.hostId === oldPlaceholderId;
